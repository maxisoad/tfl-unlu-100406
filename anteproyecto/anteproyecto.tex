...
Los sitios web de la actualidad difieren de las p'aginas de hace más de 10 a'nos, tanto en tama'no como en cantidad de recursos. Este fen'omeno hace que el protocolo ya no tenga el mismo rendimiento que en 'epocas anteriores. A pesar del avance de la tecnolog'ia en cuanto a mejoras en las velocidades de los enlaces de red, un ancho de banda grande no es el 'unico factor que interviene en la performance de la carga de las p'aginas.
...
\item Servir'a una Interfaz Web en donde se podr'a acceder a la configuración del proxy, estadísticas, logs, estado de las conexiones, informaci'on de los objetos en cach'e, etc.
\end{enumerate} 
